...
function loadAnnouncements() {
    ...
    $.ajax({
        ...
        $.each(announcements, function (index, ann) {
            var contentHTML = $.parseHTML(ann.content);
            content.append($('<div>')
...
function loadBlogs() {
    ...
    $.ajax({
        ...
        $.each(blogs, function (index, blog) {
            var contentHTML = $.parseHTML(blog.content);
            content.append($('<div>')
                    .attr({'class': 'blog'})
...
function saveAnnouncement(announcement, id) {
    if ($('#publish-date').val() === "") {
        ...
        publishDate = $('#publish-date').datepicker('getDate');
    }

    if ($('#expiry-date').val() === "") {
        var expiryDate = "3000-01-02";
    } else {
        expiryDate = $('#expiry-date').datepicker('getDate');
    }
...
function saveBlog2(blog, id) {
    if (getUserId() === 2) {
        ...
        if ($('#publish-date').val() === "") {
            ...
            pubDate = $('#publish-date').datepicker('getDate');
        }
        exDate = $('#expiry-date').datepicker('getDate');
    }
...
function submitOrPublishBlog2(blog, id) {
    if (getUserId() === 2) {
        ...
        if (pubDateField === "" || pubDateField === "3000-01-01") {
            pubDate = Date.now();
        } else {
...
function splitCategories(categories, blog, id, action) {
    ...
    for (i = 0; i < categoryArray.length; i++) {
        ...
        if (categoryArray[i] !== "" && categoryArray[i] !== "null") {
...
$('#pages-modal').on('show.bs.modal', function (event) {
    ...
    var modal = $(this);
    modal.find("#page-content").empty();
    $.ajax({
        type: 'GET',
        url: 'webpage/' + contactId
    }).success(function (page) {
        var contentHTML = $.parseHTML(page.content);
        modal.find('#pages-modal-label').text(page.title);
        modal.find("#page-content").append(contentHTML);
    });
});
